  ...
  return (
    <div className={cn('flex items-center gap-3 cursor-pointer', className)}>
      <button
        ...
        onClick={onChange}
        className={cn(
          'relative flex items-center rounded-full transition-all duration-300 ease-in-out focus:outline-none hover:scale-105 active:scale-95 cursor-pointer',
      ...
      {label && (
        <span className="text-sm text-body-100 font-medium">{label}</span>
      )}
    </div>
  );
};
